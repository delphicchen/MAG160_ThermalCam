...
# v3（2026-09-25）— 保真優先 + 實機低對比場景

## 為什麼
- v2 實機：GAN 版在白牆（溫差 < 1 °C）畫出條紋與 L 形框；只用 L1 的第一階段穩定，但 8k 次只是
  暖機，幾乎沒有超分效果。手動把顯示範圍調寬也能消除白牆紋理 → 證實是「雜訊被 p1–p99 拉滿」
  造成的輸入落在訓練分布之外。
- 38 張實拍 .mgt：溫差 0.7–33 °C（中位 5.5），像素雜訊約 0.1–0.2 °C，條紋最高約 0.1 °C；
  最低溫差那張的條紋占顯示範圍 2–5 %、雜訊 17 %，v1/v2 訓練只到 1 %（條紋）/ 12 %（雜訊）。
- 所有現有模型的能量漂移 8–13 階、對比增益 1.05–1.10：`_low_contrast` 只壓輸入不壓目標。
- 速度：手機 GPU 上 ncnn 時間跟模型本體成正比（64/16 RGB 55 ms、64/8 灰階 30 ms），
  64/10 約 37 ms，錄影時仍能 15 fps。

## 改了什麼
| 項目 | v2 | v3 |
|---|---|---|
| 退化 | FPN ≤1 % 全幅 + 只壓輸入的對比 | **感測器視角**：場景溫差 0.5–40 °C（log-uniform）、雜訊 0.03–0.20 °C、條紋 0–0.10 °C（行列對稱）、2-D 殘差 0–0.05 °C，之後輸入與目標一起套 App 的 p1–p99 拉伸 |
| 像素損失 | L1 | **L1FFTLoss**：L1 + 0.5 × 正交頻譜 L1（過平滑輸出上約占 30 %） |
| 第一階段 | 8k | **40k**（保真候選） |
| 第二階段 | 40k，感知 0.5、GAN 5e-2 | **20k**，感知 0.3、GAN 1e-2（銳利候選） |
| 模型 | 1 通道 64/8 | 1 通道 **64/10** |
| 匯出 | 只有最新一個 | **兩個候選各一個 zip**，App 選單直接匯入比較 |

## 驗證（本機 CPU torch，basicsr 用 stub）
- 沒加感測器雜訊時，目標／輸入對比比值 1.000（不再學拉對比）。
- 溫差 40 / 5 / 1 / 0.5 °C：拉伸後目標標準差 0.240 / 0.237 / 0.185 / 0.136 → 低對比場景的目標變平，
  模型學到「吵的平坦輸入 → 平坦輸出」。
- 灰階維持灰階；損失同圖為 0、梯度正常；mixin 訓練時切 1 通道、驗證時不加雜訊。
- 實拍基準（現有模型，要被 v3 打敗）：假峰值／幀 v1 57、v2-GAN 48、v2-L1 20；能量漂移 8.7 / 13.4 / 11.9 階。

## 刻意不做
- NPU：64/10 在 GPU 上已達 15 fps；畫質定案後再評估（省電、可換更大模型）。
- 16-bit 原始資料（FLIR ADAS 官方版，需註冊）。

---

# v4（2026-09-25）— 只走 L1 路線，把預算給第一階段

## v3 結果（38 張實拍 + CIDIS 20 張）
| | 捏造熱點／幀 | 溫差<2 °C | 對比增益 | PSNR（CIDIS） | 銳利度（相對 bicubic） |
|---|---|---|---|---|---|
| v3-L1FFT | 6.1 | 5.3 | 1.001 | 30.82 | ×0.50 |
| v3-lightGAN | 54.4 | 37.0 | 0.985 | 28.79 | ×1.55 |
L1FFT 採用；GAN 即使只有 1e-2 也讓捏造變 9 倍 → 不再使用。L1FFT 的弱點：毛皮等細紋理被抹成平塊（水彩感）。

## 發現
App 的時域降噪（預設開、強度 0.85）在 SR 之前，靜止畫面雜訊降到約 0.3 倍（≈0.03 °C），
v3 訓練的雜訊 0.03–0.20 °C 均勻分布，大多比實際輸入吵 → 降噪過頭。

## 改了什麼
| | v3 | v4 |
|---|---|---|
| 像素雜訊 | 0.03–0.20 °C 均勻 | **0.02–0.20 °C log-uniform**（中位數 0.11 → 0.06 °C，≤0.05 °C 的樣本 17 % → 41 %） |
| 損失 | L1 + 0.5 頻譜 | **L1 + 1.0 頻譜 + 0.5 Sobel 梯度**（約 55 / 32 / 13 %） |
| 第一階段 | 40k | **80k**（學習率 48k 減半） |
| 第二階段（GAN） | 20k | **關閉**（`RUN_STAGE2 = False`，保留作比較） |
| notebook | — | `RECIPE` 標記：Drive 上別的配方留下的實驗會被封存到 `experiments/_archive/`，不會被 `--auto_resume` 接著跑 |

## 驗證（本機）
- 模組實際抽出的雜訊：log-uniform 中位 0.062 °C、≤0.05 °C 占 41 %（均勻：0.108、17 %）。
- 損失：同圖為 0、3 通道可用、梯度正常。
- 配方標記：第一次執行封存 v3 的 net/gan 實驗；斷線重跑保留 v4 的進度。
