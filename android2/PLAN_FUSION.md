...
### P7　測距可見性與提示方式（2026-09-23）
問題：不知道 Auto 物距是量畫面哪一塊；對齊面板會自己跳出來打斷操作；±0.1（1/Z）的
範圍判定在遠距離太敏感。
- **可見光中心十字**（`fusionCrosshair`，選單可關，預設開）：融合開啟時在影像上畫出
  手機鏡頭的 AF 測距視窗 —— 中央 20% 方框（`RgbCamera.AF_CENTER_FRAC`，HAL 有接受
  `CONTROL_AF_REGIONS` 才畫方框，否則只畫十字）。位置由當幀實際配準換算：
  overlay 模式 `n_disp = (n_vis − 0.5 − d)·zoom + 0.5` → 中心落在 `0.5 − d·zoom`，
  邊長 `0.2·zoom`；Wide 模式畫面本身就是可見光，固定正中央。只畫在螢幕上，
  截圖/錄影不烙印。
- **提示改成按鈕**：超出範圍不再自動開對齊面板，改在影像下方出現一行
  `Focus ≈… is outside the saved distances` + `Align` / `✕`。按 Align 才用擬合值
  預填滑桿並開面板（`startPromptedAlign()`），✕ 等同原本的 snooze。
- **範圍判定改用公尺**：`RANGE_TOL_M = 0.5f`，比較在「公尺」而非 1/Z
  （`FusionCalibration.inRange` 先換算距離），遠距離不再因為 1/Z 一點點變化就出界；
  ∞ ± 0.5 m 仍是 ∞，只有 ∞ 對得上純 ∞ 的校正。讀值一律經 `focusToInvZ()`
  （未信任鏡頭走學到的映射），不再分 raw/focusD 兩套軸。
