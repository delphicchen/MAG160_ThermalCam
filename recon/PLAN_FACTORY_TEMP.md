...
- **未解**：既然兩邊 offset_ref 相同，縫為何原廠看不到？候選：
  (a) cali block pos0/1 的「gain maps」在 build 時被折進表裡（格式未解——u16 奇偶列
...
- **gain map 格式未解**（最後一塊拼圖）：u16 奇偶結構 65000/600、和≈65536、
  byte 拆解 (E8,FE)(97,02) 交替——所有常規解讀（Q15/Q16/8.8/s16）實測皆失敗。
  候測：可能對應不同感測讀出模式（320 子列？），或需對照真機 build 的 bufB/C
  才能反推。日後可用「真機 FFC 後的 factory 影像 vs 我們的輸出」差分反推。
