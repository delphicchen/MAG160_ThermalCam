...
print("Pixels with diff >= -100:", np.sum(diff >= -100))

# What happens if we do the exact math in enhance.py?
diff[diff == 0] = 1.0
gain = (8000.0 - 7000.0) / diff
print("Gain max:", np.max(gain))
print("Gain min:", np.min(gain))
